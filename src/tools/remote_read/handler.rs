...
use crate::utils::path::{format_with_line_numbers, normalize_remote_path, shell_escape_remote_path};
...
pub async fn handle(conn: Arc<SshConnection>, input: RemoteReadInput) -> String {
    ...
    if has_slicing {
        ...
        let end = match input.limit {
            Some(limit) => format!("{}", offset as u64 + limit),
            None => "$".to_string(),
        };
        let command = format!(
            "sed -n '{},{}p' {}",
            start, end, shell_escape_remote_path(&path),
        );
        match conn.exec(&command, Some(60_000)).await {
            Ok(result) if result.exit_code == 0 => {
                format_with_line_numbers(&result.stdout, offset)
            }
